...
const EditCompanyPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log(form);
    alert(`Company ${id} updated (frontend only)`);
  };

  return (
    <div>
      <h1 className="text-2xl font-bold mb-4 mt-20">Edit Company {id}</h1>
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-xl shadow-sm">
        ...
        <button type="submit" className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Update Company</button>
      </form>
    </div>
  );
};
